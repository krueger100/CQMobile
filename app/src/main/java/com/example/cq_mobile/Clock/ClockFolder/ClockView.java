...
public class ClockView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        gradientPaint.setShader(new RadialGradient(centerX, centerY, radius * 1.2f,
                Color.parseColor("#26346A"), Color.parseColor("#26346A"), Shader.TileMode.CLAMP));
        ...
        // Draw clock hands
        drawHand(canvas, (hour % 12) * 30 + (minute / 2), radius - 100, 10, Color.WHITE); // Hour hand
        drawHand(canvas, minute * 6, radius - 40, 6, Color.WHITE); // Minute hand
        drawHand(canvas, second * 6, radius - 20, 4, Color.parseColor("#FF007F")); // Second hand
    ...
    private void drawHand(Canvas canvas, float angle, int length, int strokeWidth, int color) {
        paint.setStrokeWidth(strokeWidth);
        paint.setColor(color);
        canvas.save();
        canvas.rotate(angle - 90, centerX, centerY); // Offset by -90° to start at 12 o'clock
